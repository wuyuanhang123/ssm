	 			  ...
	 			  alert("请选择一个需要删除的用户！");
	 			   ...
	 			   var ids = checkedBoxs.map(function(){
	 				   return this.value;
	 					   ...
	 					   window.location = "${ctx }/dept/deleteJob?ids=" + ids.get();
	 		   }
	 	   })
	    })
	</script>
		...
		<div class="row">
			...
			<div class="table-responsive">
				<form class="form-horizontal" role="form"
					action="${ctx}/job/selectJob" method="post"
					enctype="mutipart/form-data">
					<div class="form-group">
						<label for="firstname" class="col-sm-2 control-label">名字</label>
						<div class="col-xs-3">
							<input type="text" class="form-control" id="name" name="name"
								placeholder="请输入名字" value="${requestScope.job.name }">
						</div>
					</div>
					<div class="form-group">
					...
					<tbody>
						<c:forEach items="${requestScope.jobs}" var="job" varStatus="stat">
							<tr id="data_${stat.index}" class="main_trbg"
								...
								<td><a href="${ctx}/job/updateJob?flag=1&id=${job.id}">修改
								</a></td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<table class="center">
				...
				<tr valign="top">
					<td align="center" class="font3"><fkjava:pager
							...
							submitUrl="${ctx}/job/selectJob?pageIndex={0}&name=${requestScope.job.name}" />
					</td>
				</tr>
			</table>
		</div>
