		...
		<div class="row">
			...
			<div class="table-responsive">
				<form class="form-horizontal" role="form"
					action="${ctx}/document/selectDocument" method="post"
					enctype="mutipart/form-data">
					<div class="form-group">
						<label for="firstname" class="col-sm-2 control-label">标题</label>
						<div class="col-xs-3">
							<input type="text" class="form-control" id="title" name="title"
								placeholder="请输入名字" value="${requestScope.document.title }">
						</div>
					</div>
					<div class="form-group">
					...
					<tbody>
						<c:forEach items="${requestScope.documents}" var="document"
							...
							<tr id="data_${stat.index}" class="main_trbg"
								...
								<td><a
									href="${ctx}/document/updateDocument?flag=1&id=${document.id}">修改
								</a></td>
								<td><a
									href="${ctx}/document/downLoad?flag=1&id=${document.id}">下载
								</a></td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<table class="center">
				...
				<tr valign="top">
					<td align="center" class="font3"><fkjava:pager
							...
							submitUrl="${ctx}/dept/selectDept?pageIndex={0}&title=${requestScope.document.title}" />
					</td>
				</tr>
			</table>
		</div>
